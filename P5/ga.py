...
def generate_successors(population):
    ...
    # Ensure we don't start with an empty population
    if pop_size == 0:
        return population  # Prevents crash but doesn't evolve

    # Select parents for crossover
    for _ in range(pop_size):
        parent1, parent2 = random.sample(population, 2)  # Random selection
        children = parent1.generate_children(parent2)  # Crossover
        results.extend(children)  # Add children to the new generation

    return results[:pop_size]  # Ensure population size remains constant
