...
def defend_weakest_planet(state):
    ...
    def planet_strength(planet):
        incoming_friendly = sum(f.num_ships for f in state.my_fleets() if f.destination_planet == planet.ID)
        incoming_enemy = sum(f.num_ships for f in state.enemy_fleets() if f.destination_planet == planet.ID)
        return planet.num_ships + incoming_friendly - incoming_enemy
    
    weakest = min(my_planets, key=planet_strength)
    strongest = max(my_planets, key=planet_strength)
    
    if planet_strength(weakest) < 0:
        needed = abs(planet_strength(weakest)) + 10
...
def attack_enemy_weakpoint(state):
    ...
    enemy_planets.sort(key=lambda p: (p.num_ships, -p.growth_rate))
    source = my_planets[0]
    target = enemy_planets[0]
    
    needed = target.num_ships + state.distance(source.ID, target.ID) * target.growth_rate + 10
    if source.num_ships > needed * 1.2:
        return issue_order(state, source.ID, target.ID, needed)
    return False
...
def spread_to_closest_neutral_planet(state):
    ...
    for source in my_planets[:2]:  # Only use top 2 strongest planets
        for target in neutral_planets[:3]:  # Focus on top 3 valuable targets
            needed = target.num_ships + 5
            if source.num_ships > needed * 1.2:
                return issue_order(state, source.ID, target.ID, needed)
    return False
