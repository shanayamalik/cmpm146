...
def reinforce_weakest_planet(state):
    """
    Send reinforcements to the weakest planet owned by the player (fewest ships).
    """
    ...
    if strongest_planet.num_ships > weakest_planet.num_ships + 10:
        ships_to_send = (strongest_planet.num_ships - weakest_planet.num_ships) // 2
...
def spread_to_strongest_neutral_planet(state):
    """
    Expand to the neutral planet with the highest growth rate.
    """
    if len(state.my_fleets()) >= 1:
        return False
...
def spread_to_closest_neutral_planet(state):
    """
    Expand to the closest neutral planet.
    """
    if len(state.my_fleets()) >= 1:
        return False
